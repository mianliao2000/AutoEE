...
- Added FastAPI runtime in `autoee_demo/web_app.py` with state, run-demo, stop, reset, spec update, and Server-Sent Events endpoints.
- Added `autoee_demo/web_launcher.py` as the PyInstaller/web desktop entrypoint.
- Added `autoee_demo/web_state.py` to derive investor narrative, metrics, evidence badges, waveforms, loss/thermal data, design rationale, and risk summary from `DesignState`.
...
- Engineering Console now owns the detailed modular domain-pack workflow planning UI as a dark IDE / EDA-style cockpit: compact top toolbar, left Design/Test workflow and artifact/session sidebar, center Requirement/Analysis/Simulation/PCB/BOM/Review workspace, schematic-style canvas, waveform analysis panel, compact BOM table, verification checks, console output log, and right AI Design Assistant chat/execution panel. The console cockpit now uses solid muted dark surfaces, lower-contrast borders, calmer status colors, and less neon schematic/waveform styling.
- Engineering Console now includes a top-level Reset button and fits into the browser viewport with no outer page scroll; the left, center, and right panes scroll independently.
- Engineering Console now labels free-text project entry as `New Design`; sending text updates the active request and mirrors the same message into the right AI Design Assistant chat. The right chat composer now sends local messages and replies with `Error: no LLM backend connected.` until a real LLM backend is connected. Default/reset BOM tables remain empty until generated by a demo run, and Agent Version displays `v1.0`.
- Engineering Console now supports collapsible side panels: a compact left workflow rail and a right Agent rail with running/done indicators, with open/collapsed state persisted through localStorage. Focus Mode and web package export controls have been removed from the console toolbar.
- Engineering Console sidebar notices and log messages wrap long messages instead of overflowing into the center workspace.
- Engineering Console now has a persisted Day/Night segmented theme switch scoped to the technical workspace, including surface, text, divider, canvas, waveform, schematic, and status palette changes.
